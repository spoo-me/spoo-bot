...
**Tech Stack:** Python 3.12, discord.py ≥2.7.1, aiohttp, pydantic v2, aiosqlite, cryptography (Fernet), resvg-py, optional Playwright. Tooling: uv, ruff, pytest + pytest-asyncio + aioresponses.
...
- Run anything Python through uv: `uv run pytest …`, `uv run python -m spoobot`.
- Every HTTP call in the bot is aiohttp through the shared session. `requests` must not appear in `spoobot/`.
...
(`extend-exclude` keeps ruff off the legacy files until Task 21 deletes them.)
...
### Task 4: HTTP infrastructure

One place that turns aiohttp responses into typed exceptions. The client (Task 5) calls `raise_for_status_mapped` on every response.
...
from spoobot.infrastructure.http import create_session, raise_for_status_mapped


async def _serve(aiohttp_server, status: int, body: dict | None = None, headers=None):
    async def handler(request):
        return web.json_response(body or {}, status=status, headers=headers or {})
...
async def test_status_mapping(aiohttp_server, status, exc):
    server = await _serve(aiohttp_server, status, {"error": "boom", "message": "boom"})
    async with create_session() as session:
        resp = await session.get(server.make_url("/"))
        with pytest.raises(exc):
            await raise_for_status_mapped(resp)


async def test_rate_limit_includes_retry_after(aiohttp_server):
    server = await _serve(aiohttp_server, 429, {"error": "slow down"}, {"Retry-After": "13"})
    async with create_session() as session:
        resp = await session.get(server.make_url("/"))
...
async def test_2xx_passes(aiohttp_server):
    server = await _serve(aiohttp_server, 200, {"ok": True})
    async with create_session() as session:
        resp = await session.get(server.make_url("/"))
        await raise_for_status_mapped(resp)  # no raise
```

Add `pytest-aiohttp` style fixtures by adding the dev dependency:

Run: `uv add --group dev pytest-aiohttp`
...
async def _error_message(resp: aiohttp.ClientResponse) -> str:
    try:
        ...
        pass
    return f"API request failed with HTTP {resp.status}"


async def raise_for_status_mapped(resp: aiohttp.ClientResponse) -> None:
    """Map non-2xx responses to the spoobot exception hierarchy."""
    if resp.status < 400:
        return
    message = await _error_message(resp)
    if resp.status in (400, 422):
        raise ApiValidationError(message, status=resp.status)
    if resp.status == 401:
    ...
    if resp.status == 429:
        ...
        retry_after = float(retry_raw) if retry_raw and retry_raw.replace(".", "", 1).isdigit() else None
        raise RateLimitedError(message, retry_after=retry_after)
    if resp.status >= 500:
        raise ServerError(message, status=resp.status)
    raise SpooApiError(message, status=resp.status)
```

- [ ] **Step 4: Run tests to verify they pass**

Run: `uv run pytest tests/unit/test_errors_http.py -v`
Expected: 6 PASS (4 parametrized + 2)

- [ ] **Step 5: Commit**

```bash
git add spoobot/infrastructure/http.py tests/unit/test_errors_http.py pyproject.toml uv.lock
git commit -m "feat: http session factory with typed error mapping"
```
...
`tests/unit/test_spoo_client.py` — uses `aioresponses` to fake the API:
...
async def client():
    ...
    yield SpooClient(session, base_url=BASE)
    await session.close()
...
async def test_shorten_sends_bearer_when_token_given(client):
    with aioresponses() as m:
        m.post(
            f"{BASE}/api/v1/shorten",
            status=201,
            payload={"alias": "x", "short_url": f"{BASE}/x", "long_url": "https://e.com"},
        )
        await client.shorten("https://e.com", token="tok-1")
        ((_, kwargs),) = list(m.requests.values())[0], 
    # aioresponses stores request kwargs; assert the auth header went out
    key = list(m.requests.keys())[0]
    sent = m.requests[key][0].kwargs["headers"]
    assert sent["Authorization"] == "Bearer tok-1"


async def test_taken_alias_maps_to_validation_error(client):
    with aioresponses() as m:
        m.post(f"{BASE}/api/v1/shorten", status=400, payload={"message": "alias taken"})
        with pytest.raises(ApiValidationError, match="alias taken"):
            await client.shorten("https://e.com", alias="taken")


async def test_list_urls_parses_camel_case(client):
    with aioresponses() as m:
        m.get(
            f"{BASE}/api/v1/urls?page=1&pageSize=10&sortBy=created_at&sortOrder=desc",
            payload={
...
async def test_stats_series_helper(client):
    with aioresponses() as m:
        m.get(
            f"{BASE}/api/v1/stats?scope=anon&short_code=abc&group_by=browser&metrics=clicks",
            payload={
                "scope": "anon",
                "group_by": ["browser"],
                "timezone": "UTC",
...
async def test_emojify_uses_legacy_route(client):
    with aioresponses() as m:
        m.post(f"{BASE}/emoji", payload={"short_url": f"{BASE}/😀😀"})
...
class SpooClient:
    ...
    async def _json(
        ...
        *,
        ...
        async with self._session.request(
            method, url, params=params, json=json, headers=self._headers(token)
        ) as resp:
            await raise_for_status_mapped(resp)
    ...
    async def export(
        ...
        *,
        ...
        if short_code:
            params["short_code"] = short_code
        url = f"{self._base}/api/v1/export"
        async with self._session.get(url, params=params, headers=self._headers(token)) as resp:
            await raise_for_status_mapped(resp)
...
Run: `uv run pytest tests/unit/test_spoo_client.py -v`
Expected: all PASS. If the bearer-header assertion fights aioresponses' request-recording API, simplify it to use `m.requests` key inspection only — the behavior that matters is header presence.
...
async def qr():
    session = create_session()
    yield QrClient(session, base_url=BASE)
    await session.close()


async def test_gradient_png(qr):
    with aioresponses() as m:
        m.get(
            f"{BASE}/api/v1/gradient?content=https://spoo.me/abc&start=%23497dff&end=%237289da",
            body=b"\x89PNG fake",
...
async def test_classic_png(qr):
    with aioresponses() as m:
        m.get(
            f"{BASE}/api/v1/classic?content=https://spoo.me/abc&color=black&style=rounded",
            body=b"\x89PNG fake",
            content_type="image/png",
        )
...
class QrClient:
    """Client for qr.spoo.me (public, no auth)."""
...
def make_interaction_notifier(application_id: int, session) -> Callable[[LinkResult], Awaitable[None]]:
    ...
    async def notify(result: LinkResult) -> None:
        ...
        async with session.patch(url, json=payload) as resp:
            if resp.status >= 400:
                log.warning("interaction edit failed status=%s", resp.status)
...
- [ ] **Step 4: Implement `spoobot/ui/views.py`**
...
class ShareView(discord.ui.View):
    """Stats + social share link buttons for a freshly shortened URL."""

    def __init__(self, short_url: str, *, emojis: dict[str, str]) -> None:
        super().__init__(timeout=None)
        quoted = urllib.parse.quote(short_url, safe="")
        self.add_item(discord.ui.Button(label="View Statistics", url=f"{short_url}/stats" if False else f"{short_url.rsplit('/', 1)[0]}/stats/{short_url.rsplit('/', 1)[1]}"))
...
class LinkButtonView(discord.ui.View):
    """Single link button (used by /link and not-linked errors)."""
...
Fix the awkward stats-button expression in `ShareView` while here: the stats page URL is `{base}/stats/{alias}` — pass `base_url` and `alias` explicitly instead of string-splitting:

```python
class ShareView(discord.ui.View):
    def __init__(self, *, base_url: str, alias: str, emojis: dict[str, str]) -> None:
        super().__init__(timeout=None)
        short_url = f"{base_url}/{alias}"
        quoted = urllib.parse.quote(short_url, safe="")
        self.add_item(discord.ui.Button(label="View Statistics", url=f"{base_url}/stats/{alias}"))
        for name, prefix in SOCIALS.items():
            emoji_id = emojis.get(name, "")
            self.add_item(
                discord.ui.Button(
...
class SpooBot(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        ...
        self.spoo = SpooClient(self.session, base_url=cfg.spoo.api_base)
        self.qr = QrClient(self.session, base_url=cfg.spoo.qr_api_base)
        ...
        self.charts = build_renderer(cfg.charts.renderer, self.session, cfg)
        ...
        if cfg.web.enabled:
            app = make_app(
                self.auth,
                on_linked=make_interaction_notifier(self.application_id, self.session),
            )
...
def build_renderer(kind: str, session, cfg) -> ChartRenderer:
    if kind == "quickchart":
        from spoobot.services.charts.quickchart import QuickChartRenderer

        return QuickChartRenderer(session, cfg)
    if kind == "htmlcards":
...
from spoobot.ui import embeds, theme
from spoobot.ui.views import ShareView


def cooldowns(cfg):
    def deco(fn):
        for tier in (cfg.cooldowns.short_term, cfg.cooldowns.medium_term, cfg.cooldowns.long_term):
            fn = app_commands.checks.cooldown(tier.count, tier.seconds)(fn)
        return fn
    return deco
...
class Shorten(commands.Cog):
    ...
    def _result_embed(self, interaction: discord.Interaction, result: ShortUrl, *, owned: bool) -> discord.Embed:
        cfg = self.bot.config  # type: ignore[attr-defined]
        suffix = " (saved to your account)" if owned else ""
        embed = discord.Embed(
            title="URL Shortened Successfully!" + suffix,
            description="Use **/stats** with the short code to see analytics any time.",
            color=theme.PRIMARY,
            timestamp=interaction.created_at,
        )
        g1, g2 = theme.BRAND_QR_GRADIENT
        qr_url = (
            f"{cfg.spoo.qr_api_base}/gradient?text={urllib.parse.quote(result.short_url, safe='')}"
            f"&gradient1={g1}&gradient2={g2}"
        )
        embed.set_thumbnail(url=qr_url)
        embed.add_field(name="Shortened URL", value=f"```{result.short_url}```", inline=False)
        embed.add_field(name="Original URL", value=f"```{result.long_url}```", inline=False)
        return embeds.user_footer(embed, interaction.user, prefix="Shortened by")

    async def _shorten_two_tier(self, user_id: int, **kwargs) -> tuple[ShortUrl, bool]:
        ...
        try:
            result = await auth.authed_call(
                user_id, lambda token: spoo.shorten(token=token, **kwargs)
            )
            return result, True
        except NotLinkedError:
            return await spoo.shorten(**kwargs), False
    ...
    async def shorten(
        ...
        cfg = self.bot.config  # type: ignore[attr-defined]
        view = ShareView(
            base_url=cfg.spoo.api_base, alias=result.alias,
            emojis=cfg.emojis.model_dump(),
        )
        await interaction.followup.send(
            embed=self._result_embed(interaction, result, owned=owned), view=view
        )
    ...
    async def emojify(
        ...
        result = await spoo.emojify(url, emojies=emojies, max_clicks=max_clicks, password=password)
        cfg = self.bot.config  # type: ignore[attr-defined]
        view = ShareView(base_url=cfg.spoo.api_base, alias=result.alias, emojis=cfg.emojis.model_dump())
        await interaction.followup.send(
            embed=self._result_embed(interaction, result, owned=False), view=view
        )
...
async def setup(bot: commands.Bot) -> None:
    ...
    for cmd in (cog.shorten, cog.emojify):
        for tier in (cfg.cooldowns.short_term, cfg.cooldowns.medium_term, cfg.cooldowns.long_term):
            cmd.add_check(app_commands.checks.cooldown(tier.count, tier.seconds).predicate)  # type: ignore[attr-defined]
    await bot.add_cog(cog)
```

Note for the implementer: stacking three cooldown tiers via `add_check` at setup time is fiddly in discord.py — if `add_check` on the bound command resists, fall back to the plain decorator form on each command (`@app_commands.checks.cooldown(1, 10)` etc. — values from the old config: 1/10s, 5/60s, 200/86400s) and delete the `cooldowns()` helper. Behavior over cleverness; the central `CommandOnCooldown` handler renders all of them.
...
class QuickChartRenderer:
    ...
    def __init__(self, session: aiohttp.ClientSession, cfg) -> None:
        self._session = session
    ...
    async def _render(self, config: dict[str, Any]) -> bytes:
        ...
        async with self._session.post(f"{self._base}/chart", json=body) as resp:
            await raise_for_status_mapped(resp)
            return await resp.read()
...
- [ ] **Step 1: Implement `spoobot/cogs/stats.py`**
...
class StatsCog(commands.Cog):
    ...
    async def _fetch(self, interaction: discord.Interaction, short_code: str,
                     password: str | None, group_by: list[str]) -> StatsResult:
        spoo = self.bot.spoo  # type: ignore[attr-defined]
        return await spoo.stats(
    ...
    async def stats(
        ...
        await interaction.response.defer()
        result = await self._fetch(interaction, short_code, password, ["time", "country"])
        cfg = self.bot.config  # type: ignore[attr-defined]

        s = result.summary
        clicks_by_time = result.series("clicks", "time")
        spark = theme.sparkline([v for _, v in clicks_by_time][-24:])

        embed = discord.Embed(
            title=f"Stats for spoo.me/{short_code}",
            url=f"{cfg.spoo.api_base}/stats/{short_code}",
            description=f"`{spark}`" if spark else None,
            color=theme.PRIMARY,
            timestamp=interaction.created_at,
        )
        embed.add_field(name="Total Clicks", value=f"```{s.total_clicks:,}```", inline=True)
        embed.add_field(name="Unique Clicks", value=f"```{s.unique_clicks:,}```", inline=True)
        if s.last_click:
            embed.add_field(name="Last Click", value=f"```{s.last_click}```", inline=False)

        files: list[discord.File] = []
        charts = self.bot.charts  # type: ignore[attr-defined]
        if clicks_by_time:
            png = await charts.timeseries(
        ...
        view = StatsExplorerView(self, short_code, password)
        await interaction.followup.send(
            embed=embeds.user_footer(embed, interaction.user), files=files, view=view
        )
    ...
    async def short_code_autocomplete(
        ...
        spoo = self.bot.spoo  # type: ignore[attr-defined]
        try:
            page = await auth.authed_call(
                interaction.user.id,
                lambda t: spoo.list_urls(t, page=1, page_size=25, search=current or None),
            )
        except Exception:
            return []
...
class StatsExplorerView(discord.ui.View):
    """Dimension picker: renders a breakdown chart (or heatmap for country)."""
    ...
    async def pick(self, interaction: discord.Interaction, select: discord.ui.Select) -> None:
        await interaction.response.defer()
        dim = select.values[0]
        result = await self.cog._fetch(interaction, self.short_code, self.password, [dim])
        rows = result.series("clicks", dim)
        charts = self.cog.bot.charts  # type: ignore[attr-defined]
        if dim == "country":
            png = await charts.country_heatmap(dict(rows))
        else:
            png = await charts.breakdown(f"{dim.title()} — {self.short_code}", rows)
        await interaction.followup.send(
            file=discord.File(io.BytesIO(png), filename=f"{dim}.png"), ephemeral=False
        )
...
class LinksCog(commands.Cog):
    ...
    def _list_embed(self, page: UrlPage, who: discord.abc.User) -> discord.Embed:
        lines = []
        for u in page.items:
            flag = "🟢" if u.status == "ACTIVE" else "⚪"
            lines.append(f"{flag} **`{u.alias}`** · {u.total_clicks:,} clicks\n→ {u.long_url[:80]}")
        embed = discord.Embed(
            title=f"Your links — page {page.page} ({page.total} total)",
            description="\n\n".join(lines) or "No links yet. Try **/shorten**!",
            color=theme.PRIMARY,
        )
        return embeds.user_footer(embed, who)
    ...
    @group.command(name="list", description="List your shortened links 📋")
    @require_link()
    async def list_links(self, interaction: discord.Interaction, page: int = 1) -> None:
        ...
        view = LinksPager(self, page=page, has_next=result.has_next)
        await interaction.followup.send(
            embed=self._list_embed(result, interaction.user), view=view, ephemeral=True
        )
...
class LinksPager(discord.ui.View):
    ...
    async def _flip(self, interaction: discord.Interaction, new_page: int) -> None:
        ...
        await interaction.edit_original_response(
            embed=self.cog._list_embed(result, interaction.user),
            view=LinksPager(self.cog, page=new_page, has_next=result.has_next),
        )
...
- **Known simplifications (deliberate, not placeholders):** Components V2 `LayoutView` is NOT used in the first pass — classic embeds + Views ship parity faster; CV2 upgrade is the natural follow-up branch after the bake-off decides the visual direction. `/links toggle` makes an extra list call to read current status (acceptable; avoids a status param). Stats export command was deferred — `/stats` links to the web stats page which exports all formats.
- **Type consistency check:** `ChartRenderer` methods (`timeseries/breakdown/country_heatmap/close`) match both implementations and the factory; `AuthService.authed_call` signature matches every cog call site; `TokenVault` API matches AuthService and checks; `LinkResult.interaction_token` flows Task 9 → Task 10 notifier.
- **Gaps an implementer should expect:** exact `aioresponses` request-introspection API in Task 5 Step 2 may need adjusting; `resvg_py` function name may differ by version (noted in Task 16); triple-cooldown via `add_check` fallback documented in Task 14.
